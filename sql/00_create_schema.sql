-- ============================================================
-- MODELO DE DATOS - DETECTOR DE POSIBLES FRAUDES EN SINIESTROS
-- Supabase / PostgreSQL
-- ============================================================
...
-- ============================================================
-- LIMPIEZA OPCIONAL (solo ambiente de desarrollo)
-- ============================================================

DROP VIEW IF EXISTS vw_rules_scored_claims CASCADE;

DROP TABLE IF EXISTS score_siniestro CASCADE;
DROP TABLE IF EXISTS alerta_regla CASCADE;
DROP TABLE IF EXISTS variable_riesgo CASCADE;
...
DROP TABLE IF EXISTS catalogo_regla CASCADE;
DROP TABLE IF EXISTS mapeo_esquema CASCADE;
...
CREATE TABLE siniestro (
    ...
    -- 1 = fraude/estafa simulada confirmada, 0 = no fraude/estafa
...
CREATE TABLE score_siniestro (
    ...
    -- Persona 2 completa estos campos:
...
CREATE TABLE mapeo_esquema (
    ...
    created_at TIMESTAMPTZ DEFAULT NOW(),
    CONSTRAINT chk_mapeo_confidence CHECK (
        mapping_confidence IS NULL OR mapping_confidence BETWEEN 0 AND 1
    ),
    CONSTRAINT chk_mapping_origin CHECK (
        mapping_origin IS NULL OR mapping_origin IN ('llm', 'manual', 'system', 'default')
    ),
    CONSTRAINT chk_validation_status CHECK (
        validation_status IS NULL OR validation_status IN ('pending', 'valid', 'warning', 'invalid')
    )
);
...
CREATE INDEX idx_poliza_id_asegurado ON poliza(id_asegurado);
CREATE INDEX idx_vehiculo_id_asegurado ON vehiculo(id_asegurado);
CREATE INDEX idx_siniestro_id_poliza ON siniestro(id_poliza);
CREATE INDEX idx_siniestro_id_asegurado ON siniestro(id_asegurado);
CREATE INDEX idx_siniestro_id_vehiculo ON siniestro(id_vehiculo);
CREATE INDEX idx_siniestro_id_proveedor ON siniestro(id_proveedor);
CREATE INDEX idx_siniestro_fecha_ocurrencia ON siniestro(fecha_ocurrencia);
CREATE INDEX idx_siniestro_fecha_reporte ON siniestro(fecha_reporte);
CREATE INDEX idx_siniestro_cobertura ON siniestro(cobertura);
CREATE INDEX idx_siniestro_ciudad ON siniestro(ciudad);
CREATE INDEX idx_documento_id_siniestro ON documento(id_siniestro);
CREATE INDEX idx_alerta_regla_id_siniestro ON alerta_regla(id_siniestro);
CREATE INDEX idx_alerta_regla_codigo_regla ON alerta_regla(codigo_regla);
CREATE INDEX idx_score_siniestro_id_siniestro ON score_siniestro(id_siniestro);
CREATE INDEX idx_score_siniestro_nivel_riesgo ON score_siniestro(nivel_riesgo);
CREATE INDEX idx_score_siniestro_score_final ON score_siniestro(score_final);
CREATE INDEX idx_mapeo_esquema_source_file ON mapeo_esquema(source_file);

-- ============================================================
-- CATÁLOGO DE REGLAS (seed data)
-- ============================================================
...
-- ============================================================
-- ROW LEVEL SECURITY (activar sin políticas por ahora)
-- ============================================================
